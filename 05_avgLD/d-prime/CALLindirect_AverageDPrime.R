...
#read in the LD file
ld <- read.table("mt1_results_indirectnmt_CALL_ALL.tsv", header = T, sep = '\t', quote = "") %>%
...
#export avg LD tsv
write.table(avgLD, "CALLindirectnmt_avgd-prime.tsv", row.names = F, sep = '\t',
            col.names = T, quote = F)
...
ggsave("CALLindirectnmt_avgd-prime.pdf", p1, width = 7, height = 4)
...
write.table(top1, "CALLindirectnmt_topOnePercentDPrime.tsv", row.names = F, sep = '\t',
            col.names = T, quote = F)
...
write.table(top5, "CALLindirectnmt_topFivePercentDPrime.tsv", row.names = F, sep = '\t',
            col.names = T, quote = F)
