...
ggsave("CALLdirectnmt_avgr-squared.pdf", p1, width = 7, height = 4)
...
write.table(top1, "CALLdirectnmt_topOnePercentRSquared.tsv", row.names = F, sep = '\t',
            col.names = T, quote = F)
...
write.table(top5, "CALLdirectnmt_topFivePercentRSquared.tsv", row.names = F, sep = '\t',
            col.names = T, quote = F)
