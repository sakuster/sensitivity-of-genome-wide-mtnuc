...
plot1_fn <- "CHAF_mt1_LD_plot_pcor_directnmts.pdf"
#plot2_fn <- "CHAF_mt1_LD_plot_D_directnmts.pdf"

#set input filenames (modify each time)
filename_pattern <- "forLD_directnmts_par1CHAF"
filename_pattern2 <- "forLD_directnmts_par2CHAF"
...
#path_to_files <- "/Users/kusters/Documents/XiphoStartingOver_Aug2024/04_LDCalculation/fromOvis/LD_d-prime/CHAF_d-prime/CHAF_forovis/"#SAK TESTING
...
for (filename in 1:length(p1_fn_l)) {
  ...
  ##READ IN NUC ANC VALUES PER FISH AND ADD TO Par1Anc
  #SAK TESTING
  #lines <- length(par1[,1]) + 1
  ...
  #ScyDAA6.2393.HRSCAF.2888.10066505 has 281 NAs
  ...
  for (locus1 in 1:(ncol(par1) - 1)) { #- 1)) { #992 is the mt locus
    ...
    locus2 <- which(colnames(par1Anc) == "xbir.mito.45") #45 for CALL and CHAF
...
p1 <- ggplot(d, aes(x = row.names(d), y = pcor)) + 
  geom_point() + 
  labs(title = out_fn_ALL)
