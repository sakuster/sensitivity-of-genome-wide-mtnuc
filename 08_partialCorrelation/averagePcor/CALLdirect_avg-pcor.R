...
#read in the LD file
ld <- read.table("mt1_results_forLD_directnmts_par1CALLdata.tsv", 
                 header = T, sep = '\t', quote = "") %>%
...
#export avg LD tsv
write.table(avgLD, "CALLdirectnmt_avgpcor.tsv", row.names = F, sep = '\t',
            col.names = T, quote = F)
...
p1 <- ggplot(avgLD, aes(x = attribute, y = avgP_cor)) +
  ...
  labs(title = "CALL Gene Average Partial Correlation", x = "gene")

ggsave("CALLdirectnmt_avgp-cor.pdf", p1, width = 7, height = 4)
...
write.table(top1, "CALLdirectnmt_topOnePercentPcor.tsv", row.names = F, sep = '\t',
            col.names = T, quote = F)
...
write.table(top5, "CALLdirectnmt_topFivePercentPcor.tsv", row.names = F, sep = '\t',
            col.names = T, quote = F)
