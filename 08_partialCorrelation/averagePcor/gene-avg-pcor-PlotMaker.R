...
output_fn <- paste0(population, "-averagepcor-prettyPlot.pdf")
output_fn2 <- paste0(population, "-averagepcor_prettyPlot_withIncompats.pdf")
...
#make plot
all_plot <- ggplot(dat, aes(y = class, x = avgP_cor, fill = class)) + 
  geom_density_ridges() +
  theme_classic() +
  ...
  labs(title = paste0(population, " Gene Averaged LD"), 
       ...
       y = "Gene Class") +
  xlim(min(dat$avgP_cor), max(dat$avgP_cor))
...
incompat_plot <- ggplot(dat, aes(y = class, x = avgP_cor, fill = class)) + 
  geom_density_ridges() +
  scale_fill_manual(values = mypalette,
                    limits = c("non-n-mt", "non-interacting n-mt", "interacting n-mt")) +
  geom_vline(xintercept = mean(dat$avgP_cor, na.rm = TRUE), linetype = "dashed") +
  geom_point(data = incompat_genes, 
             aes(y = class, x = avgP_cor),
             show.legend=FALSE) +
...
#ggsave(path = "/Users/kusters/Library/CloudStorage/OneDrive-Colostate/Research/Xiphophorus/Writing",
       #"SuppFig_CHAF-pcor-incompatibilityPlot.pdf", plot = incompat_plot, width = 8, height = 5)
...
sumStats <- group_by(dat_df, class) %>% 
  drop_na() %>%
  summarise(count = n(), 
            mean = mean(avgP_cor, na.rm = TRUE), 
            sd = sd(avgP_cor, na.rm = TRUE))

aovDat <- dat_df %>% drop_na()
OneWayFit <- lm(avgP_cor ~ class, data = aovDat)
aov <- anova(OneWayFit)
...
topVals <- as.data.frame(head(dat[order(-dat$avgP_cor),], n = numTranscripts))
...
write.table(top1, paste0(population, "_ALLGENES_topOnePercentPcor.tsv"), col.names = T,
            row.names = F)
...
#z score calculation
avgP <- mean(dat$avgP_cor, na.rm = T)
sdP <- sd(dat$avgP_cor, na.rm = T)

zScores <- dat %>%
  right_join(geneList, by = "attribute") %>%
  mutate(Pcor_zScore = (avgP_cor - avgP) / sdP)



write.table(top1, paste0(population, "pcor-zScores.tsv"), row.names = F, sep = '\t',
            col.names = T, quote = F)


zScores %>% select(GN, avgP_cor, Pcor_zScore) %>% kable(digits = 2)
