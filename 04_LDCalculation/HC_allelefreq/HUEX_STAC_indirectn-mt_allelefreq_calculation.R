#Calculate Allele Frequency of Indirect / Non-Interacting N-mts

#set input filenames (modify each time)
filename_pattern <- "forLD_indirectnmts_par2HUEX"
filename_pattern2 <- "forLD_indirectnmts_par1HUEX"
...
all_fn <- "nucallelefreq_indirectnmts_par1HUEXSTACdata.tsv"
...
for (filename in 1:length(p1_fn_l)) {
  ...
  } #end allele frequency calculation for loop
  
  write.table(results, out_fn, sep = "\t", quote = FALSE, row.names = FALSE)
  
} #end file for loop

base_fn <- "nucallelefreq_forLD_indirectnmts"
...
pdf(file = "HC_indirect-n-mtAlleleFreqHistogram.pdf")
