...
plot1_fn <- "CALL_mt1_LD_plot_r-squared_directnmts.pdf"
...
for (filename in 1:length(p1_fn_l)) {
  ...
  for (locus1 in 1:(ncol(par1))) { #- 1)) { #992 is the mt locus
    ...
    ###RIGHT HERE, NEED TO DO R^2 CALCULATION
...
#base_fn <- "results_directnmt_CALL"
#
#result_files <- list.files(path = "mt3_CALL/", pattern = base_fn)
#
