...
outfn1 = outfn1[0] + "FILTERED_v2.tsv"

outfn2 = par2fn.split(sep = '.')
outfn2 = outfn2[0] + "FILTERED_v2.tsv"
outfn3 = "HC_ancestryResults_v2.tsv"
...
with open(par1fn, "r") as anc1, open(par2fn, "r") as anc2, open(outfn1, "w") as file1, open(outfn2, "w") as file2, open (outfn3, "w") as resultFile:
    ...
    for line1, line2 in zip(anc1, anc2):
        ...
        for i in range(1, len(modline)): #skip i = 0 bc it's fish ID, str
            ...
            if (i <= nNucSNPs - 1): 
                if (float(modline[i]) >= 0.9 and float(modline2[i]) <= 0.1):
                    genomAnc = genomAnc + 1.0
                elif (float(modline[i]) <= 0.1 and float(modline2[i]) >= 0.1):
                    par2Anc = par2Anc + 1.0
                elif (float(modline[i]) + float(modline2[i]) <= 0.2):
                    genomAnc = genomAnc + 0.5
                    par2Anc = par2Anc + 0.5
                else:
                    continue #if bad call, then skip this whole SNP
                
                
                
                #if (float(modline[i]) < 0.9 and float(modline[i]) > 0.1 ): #if par1 is not good
